...
class SimplifiedFaceGrouping:
    ...
    def load_faces_json(self, group_id):
        ...
        try:
            ...
            print(f"✅ Loaded {len(faces_dict)} faces from JSON into cache")
            return faces_dict
            
        except Exception as e:
            print(f"❌ Error loading faces JSON: {e}")
            return {}
    ...
    def mark_faces_error_batch(self, group_id, error_face_ids):
        ...
        for face_id in error_face_ids:
            self.update_face_in_cache(group_id, face_id, {'status': 'error'})
        
        print(f"✅ Marked {len(error_face_ids)} faces as error in cache")
    ...
    def update_embeddings_batch(self, group_id, updates):
        ...
        print(f"   ✅ Payload updates attempted for {len(updates)} points")
    ...
    def process_face_batch(self, group_id, batch_size=10):
        ...
        print(f"   ❌ Error faces: {len(self.error_faces)}")
    ...
    def process_unassigned_faces(self, group_id, batch_size=10):
        ...
        while True:
            ...
            if processed_batches > 100:  # Adjust as needed
                print("⚠️ Maximum batch limit reached, stopping")
                break
