...
def check_group_exists(group_id: int) -> bool:
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id FROM groups WHERE id = %s AND status = 'warming' and last_processed_step='assign_quality_score'", (group_id,))
            result = cur.fetchone()
...
class SimplifiedFaceGrouping:
    ...
    def mark_group_process_status(self , group_id , status) -> None:
    ...
    def process_all_groups(self, batch_size=10):
        ...
        if not check_group_exists(group_id):
            ...
            raise
        self.mark_group_process_status(group_id, 'healthy')
        ...
        try:
            ...
            self.mark_group_process_status(group_id, 'success')
            ...
            self.mark_group_process_status(group_id, 'failed')
            raise
